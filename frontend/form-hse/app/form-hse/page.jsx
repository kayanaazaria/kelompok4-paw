'use client'; // WAJIB: Menandakan ini adalah Client Component karena menggunakan hooks React dan API browser
...
const getAuthHeaders = () => {
    const DUMMY_JWT = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY5MThjMWRmYjI0MDlkMjY4M2IzNTFhNCIsInJvbGUiOiJoc2UiLCJ1c2VybmFtZSI6InUiLCJpYXQiOjE3MDQ5Njg2MjYsImV4cCI6MTcwNTA1NTAyNn0.8mO5xWc3L9X0QxL1aL6O-1a-P-Xq-K-q-7a-N-J-T-Y';
    
    const token = localStorage.getItem('authToken') || DUMMY_JWT; 
    return {
        headers: {
...
const FormInput = ({ label, type = 'text', name, value, onChange, options, disabled = false, isRequired = true }) => {
    // FIX 1: Menambahkan text-gray-700 sebagai warna default input
    const baseClasses = "mt-1 block w-full p-3 border rounded-lg focus:ring-2 transition duration-150 shadow-sm text-gray-700";
...
const FileUploadComponent = ({ label, fileName, handleAttachmentChange }) => {
    ...
    const handleDrop = (e) => {
        ...
        // FIX: Reset input value after drop to allow subsequent uploads
    ...
    const handleFileInputChange = (e) => {
        ...
        // FIX: Reset input value to allow the same file to be re-selected
        // and trigger the onChange event next time.
        if (fileInputRef.current) {
            fileInputRef.current.value = ''; 
        }
    };
    ...
    const fetchData = useCallback(async (id) => {
        ...
        try {
            if (id) {
                ...
                // --- PANGGILAN API NYATA DIIAKTIFKAN ---
                const data = await getLaporanDetail(id);

                setReport({ 
                    ...data,
                    // Tambahkan fallback data palsu untuk field yang tidak ada di skema utama DB
        ...
        try {
            ...
            if (report._id) {
                ...
                if (!isDraft && report.isDraft) {
                    // Jika tadinya Draft, dan sekarang dikirim
            ...
            // Logika ini akan menangkap error nyata dari backend jika gagal terhubung ke DB atau auth gagal
            const msg = error.response ? error.response.data.message : 'Terjadi kesalahan jaringan/server.';
            console.error('API Error:', msg);
            alert(`Gagal menyimpan laporan: ${msg}. Pastikan server Node.js dan Database terhubung dengan benar.`);
