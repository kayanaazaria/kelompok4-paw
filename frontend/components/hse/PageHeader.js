...
export default function PageHeader({
  title = 'Dashboard HSE',
  subtitle = 'Health, Safety & Environment Management',
  description = 'Kelola dan pantau semua laporan insiden keselamatan kerja di SOLANUM AGROTECH',
  showCreate = true,
  onCreateReport
}) {
  return (
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3 sm:mb-4">
              ...
              <div>
                ...
                <p className="text-emerald-50 text-xs sm:text-sm mt-1">{subtitle}</p>
              </div>
            </div>
            <p className="text-emerald-50 text-sm sm:text-base max-w-2xl">{description}</p>
          </div>
          {showCreate && (
            <button
